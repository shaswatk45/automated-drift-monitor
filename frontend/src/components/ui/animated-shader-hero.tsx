...
void main(){gl_Position=position;}`;
    ...
    constructor(canvas: HTMLCanvasElement, scale: number) {
      ...
      this.shaderSource = blueShaderSource;
    }
  ...
  const loop = (now: number) => {
    ...
    const coords = pointersRef.current.first;
    if (coords) {
        rendererRef.current.updateMouse(coords);
    }
  ...
  useEffect(() => {
    ...
    if (rendererRef.current.test(blueShaderSource) === null) {
      rendererRef.current.updateShader(blueShaderSource);
    }
...
// Converted Hero Component (BLUE THEME)
...
/**
 * Converted to Blue Theme - ML Drift Monitor
 */
precision highp float;
out vec4 O;
...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		vec2 p=uv;
		float d=length(p);
        // Modified vec3 to favor cyan/blue/indigo
		col+=.00125/d*(cos(sin(i)*vec3(3,2,1))+1.); 
		float b=noise(i+p+bg*1.731);
		col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
        // Deep blue atmospheric mix
